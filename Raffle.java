...
public class Raffle {
    ...
    public Raffle(ParticipantQueue kids, ListToy tl) {
    ...
    public void runRaffle() {
        ...
        ListToy tl = this.currentToys;
        PriorityQueue<Toy> prizes = new PriorityQueue<>(tl.toys.values());
        try {
            BufferedWriter log = FileIException.raffleLog();
    ...
    class QuantityCalc {
        PriorityQueue<Toy> adjustQuantityLeft(Toy t, ListToy tl, PriorityQueue<Toy> currentQueue) {
            if(t.quantity > 0){
                t.quantity -= 1;
            }
            if(t.quantity == 0){
        ...
        void removeStock(int idNum, ListToy toys) {
...
class Chance {
    ...
    Toy checkPrize(PriorityQueue<Toy> prizes, double roll) throws Exception {
     ...
     ListToy assignChance(ListToy tl){
        this.totalWeight = 0;
        this.maxChance = 0;
